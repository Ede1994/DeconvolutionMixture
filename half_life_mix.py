...
#%% Mixture
# data path and files
data_path = 'C:/Users/Eric/Documents/Python Scripts/half_life/Data/Mix/'
...
decay_sum_counts_lu = []
for block in counts:
    decay_sum_counts_iod.append(sum(block[804:809]))
    decay_sum_counts_lu.append(sum(block[56:61]))
...
A_iod, K_iod = fit_exp_linear_iod(np.asarray(ds), np.asarray(decay_sum_counts_iod))
half_life_iod = round(np.log(2)/(-K_iod*60*60*24), 1)


# use exp fit function (Lu)
A_lu, K_lu = fit_exp_linear_iod(np.asarray(ds), np.asarray(decay_sum_counts_lu))
...
ax1.plot(ds, decay_sum_counts_iod, '.', label='Counts (Iod window)')
ax1.plot(ds, decay_sum_counts_lu, 'x', label='Counts (Lu window)')
...
ax2.plot(ds, np.log(decay_sum_counts_iod), '.', label='Counts (Iod window)')
ax2.plot(ds, np.log(decay_sum_counts_lu), 'x', label='Counts (Lu window)')
ax2.plot(t, np.log(func(t, A_iod, -K_iod)), color='green', label='Fit (Iod):\n y = %0.2f e^{-ln(2)/%0.2f t}' % (A_iod,half_life_iod))
ax2.plot(t, np.log(func(t, A_lu, -K_lu)), color='red', label='Fit (Lu):\n y = %0.2f e^{-ln(2)/%0.2f t}' % (A_lu,half_life_lu))
ax2.plot(t, np.log(func(t, A_iod, -K_iod) + func(t, A_lu, -K_lu)), color='black', label='Iod + Lu')
ax2.set_xlabel('dt [s]')
