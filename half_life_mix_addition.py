...
decay_sum_counts = []
for block in counts:
    decay_sum_counts.append(sum(block[0:]))
...
A_lu, K_lu = fit_exp_linear_lu(np.asarray(ds_end), np.asarray(decay_sum_counts[3:]))
half_life_lu = round(np.log(2)/(-K_lu*60*60*24), 1)

# use exp fit function (Iod)
A_iod, K_iod = fit_exp_linear_iod(np.asarray(ds_begin), np.asarray(decay_sum_counts[0:2]), A_lu)
...
print('--- Results ---')
print('Iod activity:', (round(A_iod*0.022, 2)))
print('Lu activity:', (round(A_lu*0.005, 2)))
print('---------------')
...
ax1.plot(ds_begin, decay_sum_counts[0:2], '.', label='Counts')
ax1.plot(ds_end, decay_sum_counts[3:], 'x', label='Counts')
